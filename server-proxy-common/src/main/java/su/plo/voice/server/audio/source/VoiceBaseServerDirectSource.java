        ...
        implements BaseServerDirectSource
{
    ...
    public void setSender(@Nullable VoicePlayer player) {
        if (samePlayer(this.sender, player)) return;
        this.sender = player;
        updateSourceInfo();
    }
    ...
    public void setRelativePosition(@Nullable Pos3d position) {
        if (Objects.equals(this.relativePosition, position)) return;
    ...
    public void setLookAngle(@NotNull Pos3d position) {
        if (Objects.equals(this.lookAngle, position)) return;
    ...
    public void setCameraRelative(boolean cameraRelative) {
        if (this.cameraRelative == cameraRelative) return;
    ...
    private boolean samePlayer(@Nullable VoicePlayer first, @Nullable VoicePlayer second) {
        if (first == second) return true;
        if (first == null || second == null) return false;
        return first.getInstance().getUuid().equals(second.getInstance().getUuid());
    }
}
