...
abstract class BaseClientAudioSource<T>(
    ...
    private val diagnostics = AudioSourceDiagnostics()
    ...
    override fun process(packet: SourceAudioPacket) {
        ...
        buffer.offer(packet)
        diagnostics.recordQueued(packet, buffer.size())
    }
    ...
    override fun process(packet: SourceAudioEndPacket) {
        ...
        buffer.offer(packet)
        diagnostics.recordEndQueued(packet.sequenceNumber, buffer.size())
        endRequest?.cancel()
    ...
    private fun isAudioPacketValid(packet: SourceAudioPacket): Boolean {
        ...
        if (sourceInfo.state.diff(packet.sourceState) >= 10) {
            diagnostics.recordBadState(packet, sourceInfo.state, buffer.size())
        ...
        if (lastSequenceNumber >= 0 && packet.sequenceNumber <= lastSequenceNumber) {
            if (lastSequenceNumber - packet.sequenceNumber < 10L) {
                diagnostics.recordBadOrder(packet, lastSequenceNumber, buffer.size())
    ...
    private suspend fun processPlc() = mutex.withLock {
        ...
        diagnostics.recordPlc(sequenceNumber, buffer.size())
        ...
        if (shouldWrite()) {
            ...
            if (lastSequenceNumber >= 0) {
                ...
                if (packetsToCompensate in 1..4) {
                    diagnostics.recordGap(packet, lastSequenceNumber, packetsToCompensate, buffer.size())
            ...
            try {
                ...
                val outputSamples = if (sourceInfo.isStereo && config.advanced.stereoSourcesToMono.value()) {
                    AudioUtil.convertToMonoShorts(decoded)
                } else {
                    decoded
                }

                write(outputSamples, packet.sequenceNumber)
                diagnostics.recordDecoded(packet, outputSamples, buffer.size())
            } catch (e: EncryptionException) {
                diagnostics.recordDecodeError(packet, buffer.size())
    ...
    private suspend fun reset(cause: AudioSourceResetEvent.Cause) = mutex.withLock {
        ...
        diagnostics.recordReset(cause, buffer.size())
    }
    ...
    private inner class AudioSourceDiagnostics {

        private var nextLogAtMillis = 0L
        private var queuedPackets = 0L
        private var decodedPackets = 0L
        private var endPackets = 0L
        private var badOrderPackets = 0L
        private var badStatePackets = 0L
        ...
        private var maxGapPackets = 0
        private var lastSequenceNumberSeen = -1L
        private var lastPayloadCrc = 0L
        private var lastSourceState: Byte = 0
        private var lastPacketDistance: Short = 0
        private var lastPcmSamples = 0
        private var lastPcmPeak = 0

        fun recordQueued(packet: SourceAudioPacket, queueSize: Int) {
            queuedPackets++
            lastSequenceNumberSeen = packet.sequenceNumber
            maxQueueSize = maxQueueSize.coerceAtLeast(queueSize)
            recordPacketShape(packet)
            maybeLog("queued", packet, queueSize, false)
        }

        fun recordEndQueued(sequenceNumber: Long, queueSize: Int) {
            endPackets++
            lastSequenceNumberSeen = sequenceNumber
            maxQueueSize = maxQueueSize.coerceAtLeast(queueSize)
            maybeLog("end", null, queueSize, true)
        }

        fun recordBadState(packet: SourceAudioPacket, expectedState: Byte, queueSize: Int) {
            badStatePackets++
            recordPacketShape(packet)
            maybeLog("bad_state expected=$expectedState got=${packet.sourceState}", packet, queueSize, true)
        }

        fun recordBadOrder(packet: SourceAudioPacket, previousSequenceNumber: Long, queueSize: Int) {
            badOrderPackets++
            recordPacketShape(packet)
            maybeLog("bad_order last=$previousSequenceNumber got=${packet.sequenceNumber}", packet, queueSize, true)
        }

        fun recordGap(
            packet: SourceAudioPacket,
            previousSequenceNumber: Long,
            packetsToCompensate: Int,
            queueSize: Int
        ) {
            gapEvents++
            maxGapPackets = maxGapPackets.coerceAtLeast(packetsToCompensate)
            recordPacketShape(packet)
            maybeLog(
                "gap last=$previousSequenceNumber got=${packet.sequenceNumber} compensate=$packetsToCompensate",
                packet,
                queueSize,
                true
            )
        ...
        fun recordPlc(sequenceNumber: Long, queueSize: Int) {
            plcFrames++
            lastSequenceNumberSeen = sequenceNumber
            maybeLog("plc", null, queueSize, true)
        }

        fun recordDecoded(packet: SourceAudioPacket, samples: ShortArray, queueSize: Int) {
            decodedPackets++
            lastSequenceNumberSeen = packet.sequenceNumber
            maxQueueSize = maxQueueSize.coerceAtLeast(queueSize)
            recordPacketShape(packet)
            lastPcmSamples = samples.size
            lastPcmPeak = peak(samples)
            maybeLog("decoded", packet, queueSize, false)
        }

        fun recordDecodeError(packet: SourceAudioPacket, queueSize: Int) {
            decodeErrors++
            recordPacketShape(packet)
            maybeLog("decode_error", packet, queueSize, true)
        }

        fun recordReset(cause: AudioSourceResetEvent.Cause, queueSize: Int) {
            resets++
            maybeLog("reset=$cause", null, queueSize, true)
        }
        ...
        private fun maybeLog(
            ...
            if (!isGroupSource() || !diagnosticsEnabled() || !matchesDiagnosticsFilter()) return
            ...
            BaseVoice.DEBUG_LOGGER.log(
                "[MytrixVoice] Client group audio diag reason={} source={} line={} sender={} seq={} bytes={} crc={} source_state={} source_info_state={} packet_distance={} source_stereo={} pcm_samples={} pcm_peak={} queue={} max_queue={} queued={} decoded={} ends={} gaps={} max_gap={} bad_order={} bad_state={} plc={} decode_errors={} resets={} thread={}",
                reason,
                sourceInfo.id,
                sourceInfo.lineId,
                groupSenderId(),
                packet?.sequenceNumber ?: lastSequenceNumberSeen,
                packet?.data?.size ?: 0,
                lastPayloadCrc,
                lastSourceState,
                sourceInfo.state,
        ...
        private fun isGroupSource(): Boolean =
            sourceInfo.lineId == VoiceSourceLine.GROUP_ID ||
                    (sourceInfo as? DirectSourceInfo)?.isCameraRelative == true

        private fun groupSenderId(): UUID? =
            (sourceInfo as? DirectSourceInfo)?.sender?.id

        private fun recordPacketShape(packet: SourceAudioPacket) {
            lastPayloadCrc = checksum(packet.data)
            lastSourceState = packet.sourceState
            lastPacketDistance = packet.distance
        }

        private fun matchesDiagnosticsFilter(): Boolean {
            val playerFilter = System.getProperty("mytrixvoice.groupAudioDiag.player")
                ?.trim()
                ?.takeIf { it.isNotEmpty() }
            val senderId = groupSenderId()
            if (playerFilter != null && !senderId.toString().equals(playerFilter, ignoreCase = true)) {
                return false
            }
            ...
            val groupFilter = System.getProperty("mytrixvoice.groupAudioDiag.group")
                ...
                ?.takeIf { it.isNotEmpty() }
            if (groupFilter != null &&
                !sourceInfo.id.toString().contains(groupFilter, ignoreCase = true) &&
                !sourceInfo.lineId.toString().contains(groupFilter, ignoreCase = true)
            ) {
                return false
            }
