...
public final class ActivationTabWidget extends AbstractHotKeysTabWidget {
    ...
    private void createActivation(ClientActivation activation, boolean canInherit) {
        ...
        String controlPrefix = "activation." + activation.getName();
        if (!isAnyControlVisible(
                controlPrefix + ".type",
                "activation.type",
                controlPrefix + ".toggle_button",
                "activation.toggle_button",
                controlPrefix + ".distance",
                "activation.distance"
        )) return;

        addEntry(new CategoryEntry(McTextComponent.translatable(activation.getTranslation())));
        addControlledEntry(
                createActivationType(activation, activationConfig, canInherit),
                controlPrefix + ".type",
                "activation.type"
        );
        addControlledEntry(
                createActivationButton((VoiceClientActivation) activation),
                controlPrefix + ".toggle_button",
                "activation.toggle_button"
        );
    ...
    private void createDistance(
        ...
        if (activation.getMinDistance() == -1) {
            addControlledEntry(
                    createDistanceText(activation, activationDistance),
                    "activation." + activation.getName() + ".distance",
                    "activation.distance"
            );
        } else {
            addControlledEntry(
                    createDistanceSlider(activation, activationDistance),
                    "activation." + activation.getName() + ".distance",
                    "activation.distance"
            );
        }
