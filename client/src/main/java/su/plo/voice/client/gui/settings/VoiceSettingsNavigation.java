...
public final class VoiceSettingsNavigation implements GuiWidgetListener {
    ...
    private final List<String> tabIds = Lists.newArrayList();
    @Nullable
    ...
    public void init() {
        ...
        if (parent.isControlVisible("toggles.microphone")) {
            this.toggleMicrophoneButton = new IconButton(
                    ...
                    20,
                    (button) -> {
                        if (!parent.isControlEnabled("toggles.microphone")) return;
                        config.getVoice().getMicrophoneDisabled().invert();
                    },
                    (button, mouseX, mouseY) -> {
                        if (!parent.isControlEnabled("toggles.microphone") && parent.shouldShowServerControlledTooltip()) {
                            parent.setTooltip(parent.getServerControlledTooltip(), mouseX, mouseY);
                            return;
                        }
            ...
            toggleMicrophoneButton.setActive(parent.isControlEnabled("toggles.microphone"));
            parent.addWidget(toggleMicrophoneButton);
        } else {
            this.toggleMicrophoneButton = null;
        }

        if (parent.isControlVisible("toggles.voice")) {
            this.toggleVoiceButton = new IconButton(
                    ...
                    20,
                    (button) -> {
                        if (!parent.isControlEnabled("toggles.voice")) return;
                    ...
                    (button, mouseX, mouseY) -> {
                        if (!parent.isControlEnabled("toggles.voice") && parent.shouldShowServerControlledTooltip()) {
                            parent.setTooltip(parent.getServerControlledTooltip(), mouseX, mouseY);
                            return;
                        }
            ...
            toggleVoiceButton.setActive(parent.isControlEnabled("toggles.voice"));
            parent.addWidget(toggleVoiceButton);
        } else {
            this.toggleVoiceButton = null;
        }
    ...
    public void renderButtons(@NotNull GuiRenderContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int index = 0; index < tabButtons.size(); index++) {
            ...
            button.setActive((active == -1 || index != active) && parent.isTabEnabled(tabIds.get(index)));
        ...
        if (toggleMicrophoneButton != null) toggleMicrophoneButton.render(context, mouseX, mouseY, delta);
        if (toggleVoiceButton != null) toggleVoiceButton.render(context, mouseX, mouseY, delta);
    }
    ...
    public void addTab(
        ...
        if (!parent.isTabVisible(id)) return;
        ...
        Button tabButton = new TabButton(
                ...
                iconLocation,
                (btn) -> {
                    if (parent.isTabEnabled(id)) openTab(elementIndex);
                },
                (button, mouseX, mouseY) -> {
                    if (!parent.isTabEnabled(id) && parent.shouldShowServerControlledTooltip()) {
                        parent.setTooltip(parent.getServerControlledTooltip(), mouseX, mouseY);
                    }
                },
                true
    ...
    public boolean isMinimized() {
        ...
        int disableButtonsWidth = 14;
        if (toggleMicrophoneButton != null) disableButtonsWidth += 24;
        if (toggleVoiceButton != null) disableButtonsWidth += 24;
