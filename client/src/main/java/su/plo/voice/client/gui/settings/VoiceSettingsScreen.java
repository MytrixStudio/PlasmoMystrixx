...
public final class VoiceSettingsScreen extends GuiScreen implements GuiWidgetListener, TooltipScreen {
    ...
    public boolean isTabVisible(@NotNull String id) {
        return voiceClient.getClientControlConfig().isTabVisible(id);
    }

    public boolean isTabEnabled(@NotNull String id) {
        return voiceClient.getClientControlConfig().isTabEnabled(id);
    }

    public boolean isControlVisible(@NotNull String... ids) {
        return voiceClient.getClientControlConfig().isControlVisible(ids);
    }

    public boolean isControlEnabled(@NotNull String... ids) {
        return voiceClient.getClientControlConfig().isControlEnabled(ids);
    }

    public boolean shouldShowServerControlledTooltip() {
        return voiceClient.getClientControlConfig().shouldShowLockTooltip();
    }

    public @NotNull McTextComponent getServerControlledTooltip() {
        return McTextComponent.translatable(voiceClient.getClientControlConfig().getLockedTooltipKey());
    }
    ...
    private McTextComponent getSettingsTitle() {
        ...
        McTextComponent title = McTextComponent.translatable(
                ...
                McTextComponent.literal("Mytrix Voice"),
                McTextComponent.literal(version).withStyle(versionColor)
        );

        if (LanguageUtil.getOrDefault("gui.plasmovoice.title").split("%").length != 3) {
            return McTextComponent.literal(String.format("Mytrix Voice %s%s Settings", versionColor, version));
        }
