...
public final class OverlayTabWidget extends TabWidget {
    ...
    public void init() {
        ...
        if (isAnyControlVisible("overlay.hud_icon", "overlay.hud_position")) {
        ...
        addControlledEntry(createActivationIconPosition(), "overlay.hud_position");

        if (isAnyControlVisible("overlay.entity_icons", "overlay.static_source_icons")) {
            addEntry(new CategoryEntry(McTextComponent.translatable("gui.plasmovoice.overlay.source_icons")));
        }
        addControlledEntry(createShowIcons(), "overlay.entity_icons");
        addControlledEntry(createToggleEntry(
                McTextComponent.translatable("gui.plasmovoice.overlay.show_static_source_icons"),
                null,
                config.getOverlay().getShowStaticSourceIcons()
        ), "overlay.static_source_icons");

        if (isAnyControlVisible("overlay.enable", "overlay.position", "overlay.style")) {
        ...
        addControlledEntry(createOverlayPosition(), "overlay.position");
        addControlledEntry(createOverlayStyle(), "overlay.style");

        if (parent.isControlVisible("overlay.sources")) {
    ...
    private void createOverlaySource(@NotNull ClientSourceLine sourceLine) {
        String sourceControl = "overlay.source." + sourceLine.getName();
        if (!parent.isControlVisible(sourceControl, "overlay.sources")) return;
        ...
        if (!sourceLine.hasPlayers()) {
            ...
            addControlledEntry(new OverlaySourceEntry<>(
