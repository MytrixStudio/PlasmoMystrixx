...
import su.plo.voice.client.config.control.ClientControlConfig;
...
public final class ModServerConnection implements ServerConnection {
    ...
    public void close() {
        ...
        voiceClient.setClientControlConfig(ClientControlConfig.defaults());
        playerById.clear();

        // cleanup audio capture
        if (voiceClient.getAudioCapture() instanceof VoiceAudioCapture capture) {
            capture.setReplaceProximityWithGroup(false);
        }
    ...
    public void handle(@NotNull ConfigPacket packet) {
        ...
        voiceClient.setClientControlConfig(ClientControlConfig.parse(packet.getClientControlYaml()));
    ...
    public void handle(@NotNull SourceAudioEndPacket packet) {
        ...
        if (sources instanceof VoiceClientSourceManager sourceManager) {
            sourceManager.processAudioEndPacket(packet);
            return;
        }
    ...
    @Override
    public void handle(@NotNull GroupRoutingModePacket packet) {
        if (voiceClient.getAudioCapture() instanceof VoiceAudioCapture capture) {
            capture.setReplaceProximityWithGroup(packet.isReplaceProximityWithGroup());
        }
    }
