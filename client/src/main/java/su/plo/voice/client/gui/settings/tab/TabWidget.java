...
public abstract class TabWidget extends AbstractScrollbar<VoiceSettingsScreen> {
    ...
    protected void addControlledEntry(@NotNull Entry entry, @NotNull String... controlIds) {
        if (!parent.isControlVisible(controlIds)) return;

        if (parent.isControlEnabled(controlIds)) {
            addEntry(entry);
        } else {
            addEntry(new LockedEntry(entry));
        }
    }

    protected boolean isAnyControlVisible(@NotNull String... controlIds) {
        return Stream.of(controlIds).anyMatch(parent::isControlVisible);
    }

    private final class LockedEntry extends Entry {

        private final Entry delegate;

        private LockedEntry(@NotNull Entry delegate) {
            super(delegate.getHeight());
        ...
        public void updatePosition(@NotNull GuiRenderContext context, int index, int x, int y, int entryWidth, int mouseX, int mouseY, boolean hovered, float delta) {
            delegate.updatePosition(context, index, x, y, entryWidth, mouseX, mouseY, hovered, delta);
        }
        ...
        public void render(@NotNull GuiRenderContext context, int index, int x, int y, int entryWidth, int mouseX, int mouseY, boolean hovered, float delta) {
            delegate.render(context, index, x, y, entryWidth, mouseX, mouseY, hovered, delta);
            if (hovered && parent.shouldShowServerControlledTooltip()) {
                setTooltip(parent.getServerControlledTooltip(), mouseX, mouseY);
            }
        }
