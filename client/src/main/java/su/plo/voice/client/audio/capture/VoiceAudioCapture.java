...
public final class VoiceAudioCapture implements AudioCapture {
    ...
    private final Map<UUID, CaptureAudioDiagnostics> captureAudioDiagnostics = Maps.newConcurrentMap();
    ...
    private volatile boolean replaceProximityWithGroup;
    private boolean groupRoutingModeApplied;
    ...
    /**
     * Enables the server-managed group route. The capture thread will send the
     * microphone frame with the native GROUP activation and will not emit the
     * same frame through PROXIMITY.
     */
    public void setReplaceProximityWithGroup(boolean replaceProximityWithGroup) {
        this.replaceProximityWithGroup = replaceProximityWithGroup;
    }
            ...
            try {
                ...
                if (captureEvent.isFlushActivations()
                    ...
                    if (parentActivation.isActive()) {
                        ...
                        if (activationStreams.remove(parentActivation.getId())) {
                    ...
                    activations.getActivations().forEach((activation) -> {
                        if (activation.isActive()) {
                            activation.reset();
                            if (activationStreams.remove(activation.getId())) {
                                sendVoiceEndPacket(activation);
                            }
                        }
                    });
                    endTrackedStream(VoiceActivation.GROUP_ID);
                ...
                ClientActivation groupActivation = activations
                        .getActivationById(VoiceActivation.GROUP_ID)
                        .orElse(null);
                boolean routeParentToGroup = replaceProximityWithGroup && groupActivation != null;
                applyGroupRoutingMode(parentActivation, groupActivation, routeParentToGroup);

                if (routeParentToGroup) {
                    processActivation(device.get(), groupActivation, parentResult, samples, encoded);
                }

                boolean proximityTransitiveReached = false;
                boolean nonProximityTransitiveReached =
                        routeParentToGroup && parentResult.isActivated() && !groupActivation.isTransitive();
                ...
                for (ClientActivation activation : activations.getActivations()) {
                    if ((activation.isDisabled() && !activation.isActive()) ||
                            ...
                            (routeParentToGroup && (
                                    ...
                                    activation.getId().equals(VoiceActivation.GROUP_ID)
                            ))
                    ) continue;
                ...
                if (!routeParentToGroup && parentActivation.getId().equals(VoiceActivation.PROXIMITY_ID)) {
    ...
    private void cleanup() {
        ...
        activationStreams.clear();
        groupRoutingModeApplied = false;
    ...
    private void applyGroupRoutingMode(
            @NotNull ClientActivation parentActivation,
            @Nullable ClientActivation groupActivation,
            boolean enabled
    ) {
        if (groupRoutingModeApplied == enabled) return;

        endTrackedStream(parentActivation);
        if (groupActivation != null) {
            endTrackedStream(groupActivation);
            groupActivation.reset();
        } else {
            endTrackedStream(VoiceActivation.GROUP_ID);
        }
        ...
        if (enabled) {
            activations.getActivations().stream()
                    ...
                    .forEach(activation -> {
                        endTrackedStream(activation);
                        activation.reset();
                    });
        }

        parentActivation.reset();
        groupRoutingModeApplied = enabled;
    }

    private void endTrackedStream(@NotNull ClientActivation activation) {
        if (!activationStreams.remove(activation.getId())) return;
        sendVoiceEndPacket(activation);
    }

    private void endTrackedStream(@NotNull UUID activationId) {
        if (!activationStreams.remove(activationId)) return;
        activations.getActivationById(activationId).ifPresent(this::sendVoiceEndPacket);
    }
    ...
    private void sendVoicePacket(@NotNull ClientActivation activation,
        ...
        long sequenceNumber = getSequenceNumber(activation);
        recordCaptureDiagnostics(activation, sequenceNumber, isStereo, encoded);
    ...
    private void recordCaptureDiagnostics(@NotNull ClientActivation activation,
                                          long sequenceNumber,
                                          boolean stereo,
                                          byte[] encoded) {
        if (!audioDiagnosticsEnabled()) return;

        String activationName = activationName(activation.getId());
        String activationFilter = System.getProperty("mytrixvoice.groupAudioDiag.activation", "").trim();
        if (!activationFilter.isEmpty() &&
                !activationName.equalsIgnoreCase(activationFilter) &&
                !activation.getId().toString().equalsIgnoreCase(activationFilter)) {
            return;
        }

        captureAudioDiagnostics
                .computeIfAbsent(activation.getId(), ignored -> new CaptureAudioDiagnostics(activationName))
                .record(activation.getId(), sequenceNumber, stereo, encoded);
    }

    private static String activationName(UUID activationId) {
        if (VoiceActivation.GROUP_ID.equals(activationId)) return "group";
        if (VoiceActivation.PROXIMITY_ID.equals(activationId)) return "proximity";
        return activationId.toString();
    }

    private static boolean audioDiagnosticsEnabled() {
        return Boolean.parseBoolean(System.getProperty("mytrixvoice.groupAudioDiag.enabled", "true"));
    }
    ...
    private static final class CaptureAudioDiagnostics {

        private static final long LOG_INTERVAL_MS = 5_000L;

        private final String activationName;
        private long nextLogAtMillis;
        private long frames;
        private long lastSequenceNumber = -1L;
        private int maxPayloadBytes;
        private long lastPayloadCrc;
        private boolean lastStereo;

        private CaptureAudioDiagnostics(String activationName) {
            this.activationName = activationName;
        }

        private synchronized void record(UUID activationId, long sequenceNumber, boolean stereo, byte[] encoded) {
            frames++;
            lastSequenceNumber = sequenceNumber;
            maxPayloadBytes = Math.max(maxPayloadBytes, encoded.length);
            lastPayloadCrc = checksum(encoded);
            lastStereo = stereo;

            long now = System.currentTimeMillis();
            if (now < nextLogAtMillis) return;
            nextLogAtMillis = now + LOG_INTERVAL_MS;

            BaseVoice.DEBUG_LOGGER.log(
                    "[MytrixVoice] Client capture audio diag activation={} activation_id={} seq={} bytes={} crc={} stereo={} frames={} max_bytes={} thread={}",
                    activationName,
                    activationId,
                    lastSequenceNumber,
                    encoded.length,
